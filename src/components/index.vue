		...
		<div class="code">
			...
			<div class="code_puzzle">
				<div class="puzzle_holder">
					<div class="puzzle_bgimg" v-if="verifyBgInfo">
						<img
							:src="`http://captcha.ttz.com/${verifyBgInfo.wall}`"
							class="bgimg_bg"
						/>
						<img
			...
			<div class="code_touch">
				<div class="code_touchTxt">
					...
					<div ref="slideBtn" class="btn" @touchstart.stop="movestart"
...
<script>
   ...
   export default {
      ...
      methods: {
		...
		rangeMove(e,index) {
            ...
            if(index===1){
				...
				// 	if (this.disX > 80/scrennWidth || this.disX < 74/scrennWidth) {
				// 	this.rangeStatus = true;
				// 	ele.style.transition = '.5s all';
				// 	ele.style.transform = 'translateX(0)';
				// 	swg.style.transition = '.5s all';
      ...
      computed: {
		  verifyBgInfo(){
			//   console.log(this.$store.state.user.verifyPicInfo)
	...
	.code {
		...
		.code_title{
			...
			span:nth-child(2){
				...
				right: 0;
				top: 16px;
				background: url("http://cstaticdun.126.net//2.13.2/images/icon_light.c72616a.png") no-repeat;
		...
		.code_touch {
			...
			.code_touchTxt {
				...
				.btn {
					...
					left: 0;
					height: 100%;
                    background: url('../../assets/image/slideblock.png') 0/100% no-repeat;
